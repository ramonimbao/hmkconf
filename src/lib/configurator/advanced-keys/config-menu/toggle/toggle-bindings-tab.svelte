...
<script lang="ts">
  ...
  import { toggleConfigMenuStateContext } from "./context.svelte"
  ...
  const toggleConfigMenuState = toggleConfigMenuStateContext.get()
  const { bindingSelected } = $derived(toggleConfigMenuState)
</script>

<KeycodeAccordion
  onKeycodeSelected={(keycode) => {
    if (!bindingSelected) return
    configMenuState.updateAction({ ...action, keycode })
    toggleConfigMenuState.bindingSelected = false
  }}
  {...props}
/>
